...
# 📅 Automatically generates ~/.recon_config and ~/api_keys.json
# ════════════════════════════════════════════════════════════════════════════
...
API_KEYS_FILE="${HOME}/api_keys.json"
...
# ─── API KEYS (Optional - Can also use ~/api_keys.json) ─────────────────────
...
generate_api_keys() {
    ...
    info "You can skip this and add keys later in ~/api_keys.json"
    echo
...
# ⚠️  KEEP THIS FILE SECURE! chmod 600 api_keys.json
# ════════════════════════════════════════════════════════════════════════════
...
# 2. Run: chmod 600 ~/api_keys.json
# 3. Use with: sudo ./recon.sh target.com --api-keys ~/api_keys.json
# ════════════════════════════════════════════════════════════════════════════
EOF
    ...
    GITHUB_TOKEN=$(ask_secret "GitHub Token (leave empty to skip)")
...
install_tools() {
    ...
    if ask_yes_no "Continue with installation?"; then
        ...
        apt-get install -y -qq git curl jq wget python3 python3-pip amass nmap 2>/dev/null || \
...
show_summary() {
    ...
    echo "  sudo ./recon.sh example.com --api-keys ~/api_keys.json"
    echo
    ...
    echo "  ${YELLOW}!${RESET} Keep api_keys.json secure (chmod 600)"
...
EOF
    ...
    # Open config file in editor if available
    echo
    if ask_yes_no "Open configuration file for review?"; then
